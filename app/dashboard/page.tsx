...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [viewsResponse, clicksResponse, trafficResponse] = await Promise.all([
        fetch(`/api/dashboard/page-views?${params}`),
        fetch(`/api/dashboard/button-clicks?${params}`),
        fetch(`/api/dashboard/traffic-sources?${params}`)
      ]);
      ...
      {trafficSources.length > 0 && (
        <Card className="bg-white border-0 shadow-sm hover:shadow-md transition-shadow">
          <CardHeader>
            ...
            <p className="text-sm text-gray-500 mt-1">Where your visitors are coming from</p>
          </CardHeader>
          <CardContent>
                ...
                <div key={index} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-semibold text-gray-900">
                      {source.trafficSource === 'google_ads' ? '🎯 Google Ads' :
                       source.trafficSource === 'meta_ads' ? '📘 Meta Ads' :
                       source.trafficSource === 'linkedin_ads' ? '💼 LinkedIn Ads' :
                       source.trafficSource === 'twitter_ads' ? '🐦 Twitter Ads' :
                       source.trafficSource === 'organic_search' ? '🔍 Organic Search' :
                       source.trafficSource === 'direct' ? '🏠 Direct' :
                       source.trafficSource === 'referral' ? '🔗 Referral' :
                       source.trafficSource === 'social' ? '📱 Social' :
                       source.trafficSource === 'email' ? '📧 Email' :
                       '❓ Other'}
                    </h4>
                    <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
                      {source.totalTraffic} total
                    </span>
                  </div>
                  
                  {source.utmCampaign !== 'unknown' && (
                    <div className="text-sm text-gray-600 mb-2">
                      <strong>Campaign:</strong> {source.utmCampaign}
                    </div>
                  )}
                  
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="text-center">
                      <div className="text-lg font-bold text-blue-600">{source.pageViews}</div>
                      <div className="text-xs text-gray-500">Page Views</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-green-600">{source.buttonClicks}</div>
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    <div>Countries: {source.countries.slice(0, 2).join(', ')}{source.countries.length > 2 ? '...' : ''}</div>
                    <div>Devices: {source.devices.join(', ')}</div>
                  </div>
                </div>
              ))}
            </div>
            
            {trafficSources.length > 6 && (
              <div className="mt-4 text-center">
                <div className="text-sm text-gray-500">
                  And {trafficSources.length - 6} more traffic sources...
                </div>
              </div>
            )}
          </CardContent>
