...
const imageCache = new Map<string, ImageMapping>();
...
function isVideoUrl(value: any): boolean {
  if (typeof value !== 'string') return false;
...
function findAndReplaceImages(obj: any, path: string = ''): any {
  if (obj === null || obj === undefined) {
    return obj;
  }
  
  if (Array.isArray(obj)) {
    return obj.map((item, index) => findAndReplaceImages(item, `${path}[${index}]`));
  }
  ...
  if (typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (isImageUrl(value)) {
        ...
        result[key] = findAndReplaceImages(value, currentPath);
      } else {
        result[key] = value;
      }
    }
...
async function processImagesInObject(obj: any): Promise<any> {
  if (obj === null || obj === undefined) {
    return obj;
  }
  ...
  if (typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (isImageUrl(value)) {
        try {
          const mapping = await uploadToCloudinary(value as string, 'image');
        ...
        try {
          const mapping = await uploadToCloudinary(value as string, 'video');
          result[key] = mapping.newUrl;
          await new Promise(resolve => setTimeout(resolve, 500));
        } catch (error) {
...
function convertMongoDates(obj: any): any {
  if (obj === null || obj === undefined) {
    return obj;
  }
